...
public class PfLoadDataProxy {
    ...
    public void loadData(String familyUuid, int pageNo, boolean isPullup) {
        //如果家庭uuid为空，则返回
        if (Utils.stringIsNull(familyUuid)) {
            return;
        }
        ...
        UserRequest.getPfPicByUuid(context, familyUuid, minTime, maxTime, updateNewCountTime, pageNo, new RequestFailedResult() {
            ...
            public void result(BaseModel domain) {
                ...
                if (allPfAlbum == null || allPfAlbum.getList() == null || allPfAlbum.getList().getData() == null ||
                    ...
                    //在数据为空的时候还要从数据库查询一次，避免切换相册后不会更换数据的情况
                    loadFromSqliteByCreateTime();
                    return;
                }
...
//        List<AllPfAlbumSunObject> listTest = familyUuidObjectSql.findAll(AllPfAlbumSunObject.class);
    ...
    public List<AllPfAlbumSunObject> queryListByDateOrderBy(String family_uuid, String date, String orderBy) {

        String sql = " strftime('%Y-%m-%d'," + QUERY_CLOUMN + ") ='" + date + "' and family_uuid ='" + family_uuid + "' ";
        List<AllPfAlbumSunObject> objectList = familyUuidObjectSql.findAllByWhere(AllPfAlbumSunObject.class, sql,orderBy);
